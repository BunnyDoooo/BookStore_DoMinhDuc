...
public class DapperRepository
{
    ...
    public List<checkouts> GetAllCheckouts()
    {
        ...
        return _db.Query<checkouts>(query).AsList();
    }

    public checkouts GetCheckoutsByuserID(int id)
    {
        string query = "SELECT * FROM Checkouts WHERE id = @id";
        return _db.QueryFirstOrDefault<checkouts>(query, new { Id = id });
    }

    public checkouts GetUserInfoByUsersID(int usersID)
    {
        string query = @"
             SELECT b.id, b.Username, b.FirstName, b.LastName, b.Email, b.Address, b.PhoneNumber
             FROM Checkouts ci
             JOIN Users b ON ci.UserID = b.id
             WHERE ci.id = @UserID";

        return _db.QueryFirstOrDefault<checkouts>(query, new { UserID = usersID });
    }
    ...
    {
        string query = "DELETE FROM Checkouts WHERE id = @id";
        _db.Execute(query, new { Id = id });
    }
